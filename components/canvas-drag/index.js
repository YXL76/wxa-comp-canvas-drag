...
class DragGraph {
  constructor ({ x = 30, y = 30, w, h, type, text, fontSize = 20, color = 'red', url = null, rotate = 0, sourceId = null, selected = true }, canvas, factor) {
    if (type === 'text') {
...
DragGraph.prototype = {
  ...
  isInGraph (x, y) {
    ...
    if (x - transformScaleX >= 0 && y - transformScaleY >= 0 &&
      ...
      // 缩放区域
      return 'transform'
    } if (x - transformDelX >= 0 && y - transformDelY >= 0 &&
  ...
  insidePolygon (points, testPoint) {
    const x = testPoint[0]; const
      y = testPoint[1]
    let inside = false
    for (let i = 0, j = points.length - 1; i < points.length; j = i++) {
      const xi = points[i][0]; const
        yi = points[i][1]
      const xj = points[j][0]; const
        yj = points[j][1]
  ...
  transform (px, py, x, y, currentGraph) {
    ...
    if (this.type === 'image') {
      const resizeRito = lineB / lineA
      let newW = currentGraph.w * resizeRito
      let newH = currentGraph.h * resizeRito

      if (currentGraph.w < currentGraph.h && newW < this.MIN_WIDTH) {
        newW = this.MIN_WIDTH
        newH = this.MIN_WIDTH * currentGraph.h / currentGraph.w
      } else if (currentGraph.h < currentGraph.w && newH < this.MIN_WIDTH) {
        newH = this.MIN_WIDTH
        newW = this.MIN_WIDTH * currentGraph.w / currentGraph.h
      }

      this.w = newW
      this.h = newH
      this.x = currentGraph.x - (newW - currentGraph.w) / 2
      this.y = currentGraph.y - (newH - currentGraph.h) / 2
  ...
  methods: {
    ...
    undo () {
      ...
      if (this.data.history.length > 1) {
        ...
        const newConfigObj = this.data.history[this.data.history.length - 1]
        ...
        for (let i = 0; i < this.tempGraphArr.length; i++) {
          ...
          if (i === lastIndex) {
            ...
            if (lastDelIndex !== null && this.tempGraphArr[i].selected) {
              ...
              this.currentGraph = { ...this.tempGraphArr[lastIndex] }
            }
          } else {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.data.bgImage) {
          ...
          if (this.data.bgSourceId) {
            tmpImgConfig.sourceId = this.data.bgSourceId
          }
          exportArr.unshift(tmpImgConfig)
